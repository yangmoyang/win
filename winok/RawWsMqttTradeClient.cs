...
namespace winok
{
    public class RawWsMqttTradeClient : IDisposable
    {
        // =========================
        // 基本配置
        // =========================
        ...
        public void Dispose()
        {
            ...
            Console.WriteLine("🧹 Dispose RawWsMqttTradeClient");
    ...
    //    public event Action<string> OnDisconnected;
        ...
        private void HandleMqtt(byte[] data)
        {
           ...
           // Console.WriteLine(BitConverter.ToString(data));
            // Console.WriteLine($"dsfs！{packetType}");
            if (data.Length == 2 && data[0] == 0xD0 && data[1] == 0x00)
            {
                double cha = (DateTime.Now - _lastPing).TotalMilliseconds;
                Console.WriteLine($"差！{cha}");
                OnUserNotify?.Invoke("tiao");
            ...
            if (packetType == 0x20)
            {
                ...
                Console.WriteLine($"zaifa{_userId}");
                // SUB push_xxx
                SendWsBinary(BuildSubscribe($"push_{_userId}", 1));
                SendWsBinary(BuildSubscribe("msg_broadcast", 2));
                //SendRaw(Encoding.ASCII.GetBytes("123123"));
                //SendRaw(new byte[] { 0x82 });
                //SendRaw(new byte[] { 0x0F, 0x3D, 0xAE, 0x00, 0x0A, 0x70, 0x75, 0x73, 0x68, 0x5F, 0x32, 0x38, 0x33, 0x31, 0x35, 0x01, 0x82, 0x12, 0x3D, 0xAF, 0x00, 0x0D, 0x6D, 0x73, 0x67, 0x5F, 0x62, 0x72, 0x6F, 0x61, 0x64, 0x63, 0x61, 0x73, 0x74, 0x01 });
                //return;
            }
        ...
        private void DispatchBusiness(string topic, string payload)
        {
            try
            {
                ...
                Console.WriteLine("code", payload);
                if (obj["msg"] != null)
        ...
        private void HandleWsReadError(Exception ex)
        {
            Console.WriteLine("!!! OnUserNotify fired: " + ex.Message);
           // ForceReconnect();
            OnUserNotify?.Invoke("WS ERROR: " + ex.Message);
        ...
        public void SendRaw(byte[] mqttPacket)
        {
            ...
            {
                OnUserNotify?.Invoke("⚠️ 心跳发送失败，正在重2连");
...
//);
            ...
            //BuildWsFrame(0x82, payload);
            //if (payload[0] == 0xB7)
            //{
            //    frame = BuildWsFrame(0x32, payload);

            //}
            //else
            //{
            //    frame = BuildWsFrame(0x82, payload);
            //}
    //        if ( payload[0] == 0x0F)
    //        {
